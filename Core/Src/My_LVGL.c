...
void create_screen_1(void)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        // 为单个数据项创建容器（包含标签、值、条形图）
        lv_obj_t *item_cont = lv_obj_create(list_cont);
        lv_obj_set_size(item_cont, LV_PCT(100), 40); // 设置宽度为100%，固定高度为40像素
        lv_obj_set_flex_flow(item_cont, LV_FLEX_FLOW_ROW); // 设置为水平流式布局
        // 设置子元素对齐方式
        lv_obj_set_flex_align(item_cont, LV_FLEX_ALIGN_START, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
        lv_obj_clear_flag(item_cont, LV_OBJ_FLAG_SCROLLABLE); // 清除可滚动标志，防止与列表滚动冲突

        // 数据名称标签
        char label_text[16];
        snprintf(label_text, sizeof(label_text), "数据 %d:", i); // 格式化标签文本
        lv_obj_t *label = lv_label_create(item_cont);
        lv_label_set_text(label, label_text); // 设置标签文本
        lv_obj_set_width(label, 60); // 设置标签固定宽度

        // 数据值标签
        char value_text[8];
        snprintf(value_text, sizeof(value_text), "%d", data_values[i]); // 格式化值文本
        lv_obj_t *value_label = lv_label_create(item_cont);
        lv_label_set_text(value_label, value_text); // 设置值标签文本
        lv_obj_set_width(value_label, 40); // 设置值标签固定宽度

        // 数据可视化条形图
        lv_obj_t *bar = lv_bar_create(item_cont);
        lv_bar_set_range(bar, 0, 100); // 设置范围为0-100
        lv_bar_set_value(bar, data_values[i], LV_ANIM_OFF); // 设置当前值，关闭动画
        lv_obj_set_flex_grow(bar, 1); // 条形图占据剩余空间
        // 应用样式：背景样式应用于PART_MAIN，指示器样式应用于PART_INDICATOR
 ...
 */
void add_data_to_chart_screen_3(int32_t new_point1, int32_t new_point2)
{
